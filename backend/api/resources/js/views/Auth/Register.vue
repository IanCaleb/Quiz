...
<template>
  <div class="max-w-md mx-auto bg-white p-6 rounded shadow">
    <h2 class="text-2xl font-semibold text-primary mb-4">Registrar</h2>

    <div v-if="error" class="mb-3 text-sm text-red-600">{{ error }}</div>

    <label class="block mb-2 text-sm">Nome</label>
    <input v-model="name" type="text" class="w-full border rounded px-3 py-2 mb-3" />
    ...
    <div class="flex items-center justify-between">
      ...
      <router-link to="/login" class="text-sm text-secondary">Já tem conta?</router-link>
    </div>
  </div>
</template>
